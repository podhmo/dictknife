...
def link(src):
    ...
    def on_has_ref(walker, resolver, d):
        ...
        if not file_path:
            if accessor.maybe_access_container(cache[resolver.path], ref_path) is None:
                sys.stderr.write("where {}: {!r} is not found\n".format(resolver.path, d["$ref"]))
        else:
            ...
            if accessor.maybe_access_container(data, ref_path) is None:
                sys.stderr.write("where {}: {!r} is not found\n".format(resolver.path, d["$ref"]))
